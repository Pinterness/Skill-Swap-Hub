...
router.put('/profile', authMiddleware, async (req, res) => {
    try {
        const { username, skills, avatar } = req.body;
...
router.post('/certificate', authMiddleware, async (req, res) => {
    try {
        ...
        if (!name) return res.status(400).json({ success: false, message: 'Tên bằng cấp là bắt buộc' });

        const user = await User.findByIdAndUpdate(
            req.user.id,
            { $push: { certificates: { name, issuer, issueDate, expiryDate, credentialUrl } } },
            { new: true }
        ).select('-password');

        res.json({ success: true, message: 'Đã thêm bằng cấp', certificates: user.certificates });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Lỗi hệ thống' });
    }
});
...
router.delete('/certificate/:certId', authMiddleware, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Đã xóa bằng cấp', certificates: user.certificates });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Lỗi hệ thống' });
    }
});
...
router.get('/:userId/certificates', async (req, res) => {
    try {
        const user = await User.findById(req.params.userId).select('certificates username');
