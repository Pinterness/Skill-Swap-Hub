...
export default function AuthForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin && !EMAIL_REGEX.test(email.trim())) {
      setError("Vui lòng nhập email hợp lệ, ví dụ: tenban@gmail.com");
      return;
    }
    ...
    try {
      if (isLogin) {
        ...
        setVerificationEmail(res.data.email || email.trim());
        setOtp("");
      ...
      if (err.response?.data?.code === "EMAIL_NOT_VERIFIED") {
        ...
        setOtp("");
        setError("Tài khoản này chưa xác thực email. Hãy nhập mã đã nhận.");
      } else {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Không thể xác thực email");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto relative z-10">
      ...
      >
        ...
        {success && (
          <div className="mb-4 px-4 py-3 rounded-xl bg-green-500/10 border border-green-500/30 text-green-600 dark:text-green-400 text-sm text-center">
            {success}
          </div>
        )}
        ...
        {showVerification ? (
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-12 pl-12 pr-4 bg-background border border-border rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-foreground text-sm tracking-[0.35em]"
                required
              />
            </div>
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
            >
              {loading ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : (
                <>
                  Xác thực email <ArrowRight className="w-4 h-4" />
                </>
              )}
            </button>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 mt-6 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-all shadow-[0_0_20px_rgba(255,107,74,0.15)] hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : (
